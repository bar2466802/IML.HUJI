...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        for idx in range(self.max_iter_):
            gradient = f.compute_jacobian(X=X, y=y)  # Get the current gradient
            ...
            self.callback_(solver=self, weights=f.weights, val=f.compute_output(X=X, y=y), grad=gradient, t=idx,
                           eta=eta, delta=delta)
